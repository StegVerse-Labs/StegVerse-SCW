#!/usr/bin/env python3
"""
StegVerse-SCW AutoPatch (workflow YAML fixer)

- Scans .github/workflows/*.yml / *.yaml in this repo
- Applies YAML-00x fixes based on patches/manifest.json
- Writes:
    self_healing_out/AUTOPATCH_REPORT.json
    self_healing_out/AUTOPATCH_REPORT.md
- Prints a JSON summary to stdout (for GitHub Actions logs),
  plus per-file logging so you can see exactly what it touched.
"""
...
def load_manifest():
    """Load patches/manifest.json if present, else return a sane default."""
    ...
    try:
        ...
        return {
            ...
            "workflows": {},
            "fixes": {},
            "_error": f"bad manifest: {e}",
        }


def list_workflows():
    """Return a sorted list of workflow YAML files in .github/workflows."""
...
def ensure_permissions(txt, perms):
    """
    If there is no explicit 'permissions:' block, inject one using the
    provided perms list (e.g. ['contents: write', 'actions: write']).
    """
...
def ensure_concurrency(txt, group, cancel):
    """Ensure a concurrency block exists; if not, inject one."""
...
def ensure_push_triggers(txt, paths):
    """
    Add a push trigger block referencing the given paths if not already present.
...
def ensure_read_intent_step(txt):
    """Inject the 'Read run intent (RID/TASK)' step after checkout if missing."""
...
def ensure_upload_sweep_step(txt):
    """Ensure an 'Upload Sweep (reusable)' step exists."""
...
def rewrite_printf_to_heredoc(txt):
    """
    Replace fragile printf '%s\n' blocks used to generate files with a safer heredoc.
    Target pattern is:
        printf '%s\n' 'line1' 'line2' > path
    """
    changed = False
...
# ---------------------------------------------------------------------------
# Apply fixes to a single workflow file
# ---------------------------------------------------------------------------

def apply_fixes(internal_name, yaml_text, manifest):
    """
    Apply configured fixes from the manifest to a single workflow YAML.
    internal_name is typically name.lower().replace(" ", "_").
    """
    changed = []
    notes = []
...
def detect_name(yaml_text, path):
    """Try to detect workflow name: from `name:` line, else from file stem."""
...
def main():
    print("=== StegVerse-SCW AutoPatch (workflow YAML) ===")
    print(f"ROOT: {ROOT}")
    print(f"RID : {RID}")
    print(f"TASK: {TASK}")
    print()
    ...
    if "_error" in manifest:
        print(f"Warning: manifest error -> {manifest['_error']}")
    ...
    print(f"Found {len(workflows)} workflow file(s) in .github/workflows")
    print()
    ...
    for wf in workflows:
        try:
            ...
            print(f"[ERROR] Could not read {wf}: {e}")
        ...
        print(f"[AutoPatch] Scanning workflow: {wf.as_posix()} (name={name}, internal={internal})")

        new_txt, fixes_applied, _ = apply_fixes(internal, txt, manifest)
        changed = bool(fixes_applied)

        if changed:
            try:
                wf.write_text(new_txt, encoding="utf-8")
                print(f"  -> CHANGED, fixes applied: {', '.join(fixes_applied)}")
            except Exception as e:
    ...
    # Write JSON report
    (OUT / "AUTOPATCH_REPORT.json").write_text(
    ...
    for r in report["results"]:
        ...
        md.append(
            f"- `{r['path']}` — {'CHANGED' if r['changed'] else 'OK'} — fixes: {fixes}"
        )

    (OUT / "AUTOPATCH_REPORT.md").write_text("\n".join(md), encoding="utf-8")

    print()
    print("=== AutoPatch Summary ===")
